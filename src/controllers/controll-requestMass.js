...
const MassSchedule = require('../models/massSchedule'); // <-- ✨ ASEGÚRATE QUE ESTE ESTÉ
...
          }
          ...
          // --- ✨ NUEVO: 2. ACTUALIZAR EL HORARIO EN EL CALENDARIO ---
          // Buscar la programación de misas para la fecha
          const schedule = await MassSchedule.findOne({ date });
          if (!schedule) {
            return res.status(404).json({ message: 'No se encontró una programación de misas para esta fecha' });
          }
...
          const timeSlotIndex = schedule.timeSlots.findIndex(slot => slot.time === time);
          if (timeSlotIndex === -1) {
            return res.status(404).json({ message: 'El horario especificado no existe en la programación' });
          }

          const timeSlot = schedule.timeSlots[timeSlotIndex];
          // Verificar si ya está ocupado
          if (timeSlot.status === 'Ocupado') {
            return res.status(400).json({ message: 'El horario ya está ocupado por otra misa.' });
          }

          // 3. Buscar o crear al usuario
          let user = await userModel.findOne({ documentNumber: documentNumber });
    
          if (!user) {
            if (!name || !lastName || !mail || !birthdate || !typeDocument) {
              return res.status(400).json({ message: "Faltan datos del solicitante para crear el nuevo usuario." });
            }
...
            time,
            intention,
            status: 'Confirmada' // Confirmada de una vez
          });
          const savedRequest = await newMassRequest.save();

          // --- ✨ NUEVO: 5. ACTUALIZAR EL HORARIO A 'OCUPADO' ---
          schedule.timeSlots[timeSlotIndex].status = 'Ocupado';
          schedule.timeSlots[timeSlotIndex].available = false;
          schedule.timeSlots[timeSlotIndex].reservedBy = savedRequest._id;
          await schedule.save();
...
          console.log('✅ Misa, Horario Ocupado y Pago Manual creados:', newPayment.referenceCode);
